...
def steward(config_file_path:str, ukb_gatk4_base_quality_score_recalibrator_path:str, send_message_script:str) -> None:
    ...
    params_d = {
        ...
        'known_vcf'          : config_d['ukbParams']['known_vcf'],
        'known_vcf_tbi'      : config_d['ukbParams']['known_vcf_tbi'],
        'known_indels'       : config_d['ukbParams']['known_indels'],
        'known_indels_tbi'   : config_d['ukbParams']['known_indels_tbi'],
        'standard_indels'    : config_d['ukbParams']['standard_indels'],
        'standard_indels_tbi': config_d['ukbParams']['standard_indels_tbi']
    ...
    ukb_gatk4_base_quality_score_recalibrator_command = 'nextflow run -offline -profile singularity -bg -params-file {} {} >> run_log.txt'.format(params_file_path, ukb_gatk4_base_quality_score_recalibrator_path)
    return_value = os.system(ukb_gatk4_base_quality_score_recalibrator_command)
    logging.info(ukb_gatk4_base_quality_score_recalibrator_command)
    ...
    while True:
        with open(execution_trace_file, encoding = 'UTF-8') as trace_f:
            for line in trace_f:
                if 'GATK4_BASERECALIBRATOR' in line:
                    if 'COMPLETED' in line:
                        feedback_dict['endDate'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                        feedback_dict['error'] = 0
                        for file in os.listdir('{}/results'.format(analysis_path)):
                            if file.endswith('.table'):
                                table_file = '{}/results/{}'.format(analysis_path, file)
                        report_download_dict = {
                            ...
                            'text' : [
                                {
                                    'sort'   : 1,
                                    'title'  : 'GATK4 Base Quality Score Recalibrator进行碱基质量校正结果',
                                    'content': 'GATK4 Base Quality Score Recalibrator进行碱基质量校正结果文件：#&{}'.format(table_file),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_gatk4_base_quality_score_recalibrator_path', required=True, help='the full path of the ukb_gatk4_base_quality_score_recalibrator')
    ...
    ukb_gatk4_base_quality_score_recalibrator_path = args.ukb_gatk4_base_quality_score_recalibrator_path
    send_message_script = args.send_message_script
    # logging
    log_file = '{}/ukb_gatk4_base_quality_score_recalibrator.log'.format(os.path.dirname(config_file_path))
    logging.basicConfig(
        filename=log_file, 
        level=logging.INFO, 
    ...
    steward(config_file_path, ukb_gatk4_base_quality_score_recalibrator_path, send_message_script)
